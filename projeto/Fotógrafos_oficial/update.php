...
$query = "SELECT 'nome_usuario', 'nome_completo', 'telefone', 'cidade', 'email', 'senha', 'biografia', 'fotos_perfil', 'fotos', 'face', 'insta', 'linkedin', 'cod_usuario' FROM cadastro WHERE cod_usuario = ? LIMIT
0,1";
...
$query = "UPDATE cadastro SET nome_usuario=:nickname, nome_completo=:nome, telefone=:telefone, cidade=:cidade, 
email=:email, senha=:senha, biografia=:biografia, fotos_perfil=:fotos_perfil, fotos=:fotos, face=:face, 
insta=:insta, linkedin=:linkedin WHERE cod_usuario=:cod_usuario";
...
$biografia = htmlspecialchars(strip_tags($_POST['biografia']));
$fotosPerfil = htmlspecialchars(strip_tags($_POST['fotos_perfil']));
$fotos = htmlspecialchars(strip_tags($_POST['fotos']));
$face = htmlspecialchars(strip_tags($_POST['face']));
$insta = htmlspecialchars(strip_tags($_POST['insta']));
$linkedin = htmlspecialchars(strip_tags($_POST['linkedin']));

// bind the parameters
$stmt->bindParam(':nickname', $nome_usuario);
...
$stmt->bindParam(':fotos_perfil', $fotosPerfil);
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] ."?cod_usuario=$cod_usuario");?>" method="post">
  ...
  <input type="tel" name="telefone" placeholder="Formato: (xx) xxxxx-xxxx" required="" value="<?php print $_SESSION['telefone'] ?>">
  <p>Cidade</p>
  ...
  <p>Biografia</p>
  <input type="textarea" name="biografia" placeholder="Digite uma breve biografia" required="" value="<?php print $_SESSION['biografia'] ?>">
  <p>Selecione fotos de perfil</p>
  <input type="file" name="fotos_perfil" required="" value="<?php print $_SESSION['fotos_perfil'] ?>">
  <p>Exiba seu portifólio</p>
  <input type="file" name="fotos" required="" value="<?php print $_SESSION['fotos'] ?>">
  <p>Cole o URL do seu Facebook</p>
  <input type="text" name="face" required="" value="<?php print $_SESSION['face'] ?>">
  <p>Cole o URL do seu Instagram</p>
  <input type="text" name="insta" required="" value="<?php print $_SESSION['insta'] ?>">
